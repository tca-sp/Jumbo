...
func New_mvba(node_num int, K int, id int, round int, sendMsgCH chan pb.SendMsg, bamshCH chan pb.BAMsg, rbcmsgCH chan pb.RBCMsg, check_input_rbc func([]byte, []bool, chan bool) bool, lastcommit []byte, input []byte, outputCH chan []byte, broadcastheights []bool) *MVBA {
	...
	for i := 0; i < node_num; i++ {
		mvba.rbcmsgCHs[i] = make(chan pb.RBCMsg, 1000)
	}
...
func (mvba *MVBA) Launch() {
	...
	oldbabuf := make(chan pb.BAMsg, 1000)
	newbabuf := make(chan pb.BAMsg, 1000)

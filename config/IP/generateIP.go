...
func main() {
	...
	//generate txpool middle IP
	TxpoolIPmPath := fmt.Sprintf("%s%sipm.txt", gopath, gen.TpIPPath)
	f, err = os.OpenFile(TxpoolIPmPath, os.O_RDWR|os.O_CREATE|os.O_TRUNC, 0777)
	if err != nil {
		fmt.Printf("Error: %s\n", err)
		return
	}

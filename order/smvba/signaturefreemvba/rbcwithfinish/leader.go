...
func (bcl *BC_l) handle_msgin() {
	...
	for {
		...
		case 2:
			//get a echo msg
			////fmt.Println("get a echo msg from ", rbcmsg.ID, "of height", rbcmsg.Round)
			bcl.echoCH <- rbcmsg

		case 3:
			//get a ready msg
			////fmt.Println("get a ready msg from ", rbcmsg.ID, "of height", rbcmsg.Round)
			bcl.readyCH <- rbcmsg
...
func (bcl *BC_l) send_val() {
	//fmt.Println("send val msg of height", bcl.round)
	for i := 0; i < bcl.num; i++ {
	...
	//fmt.Println("send val msg of height", bcl.round, "done")
...
func (bcl *BC_l) send_echo() {
	...
	////fmt.Println("send echo msg of height", height)
	for i := 0; i < bcl.num; i++ {
		if i+1 != bcl.nid {
			sendmsg := pb.SendMsg{
				ID:   i + 1,
...
func (bcl *BC_l) send_ready() {
	...
	////fmt.Println("send ready msg of height", height)
	for i := 0; i < bcl.num; i++ {
		if i+1 == bcl.nid {
			bcl.readyCH <- msg
		} else {

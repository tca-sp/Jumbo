...
type MVBA struct {
	...
	input       []byte      //rcv msg from order_m
...
*/
func New_mvba(node_num int, K int, id int, round int, sendMsgCH chan pb.SendMsg, sigmeta cy.Signature, check_input func([]byte, []byte, int, cy.Signature, [][][]pb.BCBlock) bool, check_input_rbc func([]byte, [][]int32, chan bool) bool, check_inputs_QCagg func([]byte, []byte, int, cy.Signature) bool, lastcommit []byte, input []byte, sendCH chan pb.Msg, msgCH chan pb.Msg, MVBARoud int, broadcasttype string, broadcastheights [][]int32, oldblocks [][][]pb.BCBlock) *MVBA {
	...
	mvba := &MVBA{
		...
		Halt_CH: make(chan pb.Msg, 10000),

		MVBA_done: make(chan bool),

		Pi_buff:    make(chan pb.Msg, node_num),
		haltCH:     make(chan pb.Msg, node_num),
		LoopMsgCH:  make(chan pb.Msg, 10000),
		LoopMsgBuf: make(chan pb.Msg, 10000),
		DMMVBARoud: MVBARoud, //round number in dumbo mvba
	...
	case "CBC_QCagg":
		mvba.check_inputs_QCagg = check_inputs_QCagg
	default:
...
func (mvba *MVBA) Launch() {
	...
	go mvba.handle_halt(mvba.Roundmvba, mvba.DMMVBARoud) //deal halt msg (for we need to deal with old loop halt msg)
	...
	for {
		...
		loop_info := &loopInfo{
			...
			PBC1_CH:       make([]chan pb.Msg, mvba.Node_num), //input
			PBC2_CH:       make(chan pb.Msg, 4000),            //back sign
			PBC3_CH:       make([]chan pb.Msg, mvba.Node_num), //combined sign

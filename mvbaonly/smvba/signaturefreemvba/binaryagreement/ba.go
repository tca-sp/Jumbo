...
func NewBA(num int, id int, mvbaround int, baround int, input bool, secondinput chan bool, outputCH chan bool, msgin chan pb.BAMsg, msgout chan pb.SendMsg) *BA {
	...
	return &BA{
		...
		finishCH: make(chan pb.BAMsg, 500),
...
func (ba *BA) Launch() {
	...
	oldloopbuf = make(chan pb.BAMsg, 500)
	newloopbuf = make(chan pb.BAMsg, 500)
	for {
		...
		loopinfo := &LoopInfo{
			...
			input:     input,
			values:    make([]bool, 2),
			valCH:     make(chan pb.BAMsg, 500),
			auxCH:     make(chan pb.BAMsg, 500),
			confCH:    make(chan pb.BAMsg, 500),
		...
		newloopbuf = make(chan pb.BAMsg, 10000)
...
//			fmt.Println("tmp conf signal of loop ", loopinfo.loop, ":", v0_conf, v1_conf, a0, a1, amix)
		...
		if loopinfo.values[0] && loopinfo.values[1] {
			fmt.Println("failed to commit, start next loop")
			input = coin
			ba.loop++

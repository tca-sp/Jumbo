...
func (mvba *MVBA) handle_loop_msg(oldbuf chan pb.Msg, newbuf chan pb.Msg, loop_info *loopInfo, loop int) {
	...
	for {
		...
		if msg.RawMsg.Round < int32(loop_info.round) {
			continue
		} else if msg.RawMsg.Round > int32(loop_info.round) {
			fmt.Println("get a future round msg")
			panic("wrong msg in handle loop")
		}
...
func (mvba *MVBA) handle_PBC(leader int, loop_info *loopInfo) {
	...
	case "CBC_QCagg":
		res := mvba.check_inputs_QCagg(PBC1msg.RawMsg.Values, mvba.lastCommit, mvba.Node_num, mvba.sigmeta)
		if !res {
			panic("wrong input in PBC1")
			return
		}
...
func (mvba *MVBA) handle_mine_PBC(loop_info *loopInfo) {
	...
	//loop_info.pis[mvba.ID-1] = append(loop_info.pis[mvba.ID-1], &myPBC4_sign)
...
func (mvba *MVBA) handle_pi(loop_info *loopInfo) {
	...
	for {
		...
		if !mvba.verifySigns_RawMsg(Pi_RawMsg, *Pimsg.SS) {
			panic(fmt.Sprintln("wrong pis signs for pi", Pi_RawMsg))
			return
		}
